        ...
        if hasattr(event.artist, 'get_label'):
            ...
            for key, (line, _, _, _) in self.lines.items():
                if line == event.artist:
                    self.line_clicked.emit(('moments', key))
                    return
            # Check moments_plotter lines
            for key, (line, _, _, _, _, _) in self.moments_plotter.lines.items():
                if line == event.artist:
                    self.line_clicked.emit(('moments', key))
                    return
...
class MomentsTab(QWidget):
            ...
            for group in groups:
                for component in ['x', 'y', 'z']:
                    ...
                    if use_gait_cycle:
                        ...
                        for pct in [self.foot_off_pcts.get('left'), self.foot_off_pcts.get('right')]:
                            if pct is not None:
                                plot_widget.ax.axvline(x=pct, color='grey', linestyle='--', linewidth=2, alpha=0.6)
    ...
    def on_line_clicked(self, line_info):
        ...
        if self.highlighted_line == line_info:
            # De-highlight
            if plotter_type == 'moments':
                self.moments_plotter.highlight_line(key, highlight=False)
            ...
            if self.highlighted_line is not None:
                ...
                if prev_plotter_type == 'moments':
                    self.moments_plotter.highlight_line(prev_key, highlight=False)
                elif prev_plotter_type == 'gait':
                    self.gait_cycle_plotter.highlight_line(prev_key, highlight=False)

            # Highlight new line
            if plotter_type == 'moments':
                self.moments_plotter.highlight_line(key, highlight=True)
            elif plotter_type == 'gait':
            ...
            # Update display info
            if plotter_type == 'moments':
                info = self.moments_plotter.get_line_info(key, self.current_frame, self.gait_cycles)
            elif plotter_type == 'gait':
                info = self.gait_cycle_plotter.get_line_info(key, self.current_frame, self.gait_cycles, 'MOMENTS')
